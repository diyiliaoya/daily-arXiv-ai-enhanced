...
TL;DR: 利用地面和空间观测监测和预测3I/ATLAS彗星的光学特性，特别是通过空间太阳日冕仪和PUNCH任务在2025年10月8日至30日期间跟踪其光学星等。


<details>
  <summary>Details</summary>
Motivation: 促进行星际航天器对3I/ATLAS彗星的观测，特别是在其不可见期间保持对其光学特性的跟踪。

Method: 结合地面和空间观测，使用空间太阳日冕仪和PUNCH任务数据监测彗星光度变化。

Result: 成功在整个近日点通过期间跟踪了3I/ATLAS的光学星等，包括地球不可见时期。

Conclusion: 空间观测方法能够有效补充地面观测的局限性，确保对彗星光学特性的连续监测。
...
TL;DR: 使用ALMA望远镜在星际彗星3I/ATLAS中检测到甲醇和氰化氢分子，发现它们的释放模式不同，甲醇可能来自彗发内部来源而非纯粹核体升华，且甲醇/氰化氢比例异常高。


<details>
  <summary>Details</summary>
Motivation: 研究星际彗星3I/ATLAS的分子组成和释放机制，以了解其化学特性和与其他彗星的差异。

Method: 使用ALMA的Atacama Compact Array在2025年8-10月期间对彗星进行观测，分析甲醇和氰化氢的释放模式和产生率。

Result: 甲醇和氰化氢表现出不同的释放模式，甲醇可能来自彗发内部来源，其产生率在接近2天文单位时急剧增加。甲醇/氰化氢比例高达124和79，是迄今测量到的最高值之一。

Conclusion: 3I/ATLAS彗星具有独特的化学特征，甲醇可能来自彗发内部过程而非纯粹核体升华，其异常高的甲醇/氰化氢比例表明其化学组成与大多数太阳系彗星不同。
...
TL;DR: 本研究利用JWST观测数据分析了COCONUTS-2系统，这是一个由T9棕矮星和M3恒星组成的宽双星系统。通过光谱和测光数据，确定了该系统属于大熊座日冕星群（414±23 Myr），并精确计算了COCONUTS-2b的物理参数，支持其为一颗行星质量天体（7.5±0.4 MJup）。


<details>
  <summary>Details</summary>
Motivation: 研究冷棕矮星对于理解冷巨系外行星大气的多样性特征至关重要。COCONUTS-2系统提供了一个独特的机会来表征一个冷基准棕矮星。

Method: 使用JWST的NIRSpec G395H光谱（R~2700，2.87-5.13 um）和MIRI F1000W、F1280W、F1800W测光数据，结合BANYAN Sigma分析系统运动学，重新估计COCONUTS-2b的天体测量参数。

Result: 发现系统有99%概率属于大熊座日冕星群（414±23 Myr），计算出COCONUTS-2b为行星质量天体（7.5±0.4 MJup），有效温度、表面重力和半径等参数。与另一个候选成员0825+2805（Y0.5棕矮星，3.7±0.2 MJup）比较发现吸收特征差异，表明后者可能有更强的垂直混合。

Conclusion: COCONUTS-2b是一个行星质量天体，可能通过与恒星相同的机制形成。该研究为理解冷棕矮星和系外行星提供了重要基准数据。
...
TL;DR: 通过高分辨率3D/2D流体动力学模拟研究发现，吸积行星的迁移行为与传统非吸积行星不同，中等质量行星（质量比1×10^-4到4×10^-3）表现出向外迁移趋势，这主要归因于吸积行星周围方位角不对称结构产生的正扭矩。


<details>
  <summary>Details</summary>
Motivation: 传统非吸积行星迁移理论中，I型和II型迁移通常导致向内迁移，这与行星系统架构难以匹配。本研究旨在探索吸积行星的迁移行为，以解释行星系统形成的多样性。

Method: 采用系统性的高分辨率3D/2D数值流体动力学模拟，在不同盘条件下比较不同质量比行星的动力学演化。

Result: 发现行星吸积能显著减弱向内迁移趋势甚至改变迁移方向：低/高质量行星分别表现为I/II型向内迁移，而中等质量行星（吸积最强）表现出向外迁移趋势。向外迁移主要由吸积行星周围方位角不对称结构产生的正扭矩驱动。

Conclusion: 行星质量增长的终止等同于从向外迁移向向内迁移的转变。向外迁移的质量比范围受盘粘度和盘高宽比影响，简单的间隙开启参数可以确定向外迁移条件。
...
<details>
  <summary>Details</summary>
Motivation: 随着JWST的高精度观测能力，需要定量比较一维和三维大气模型的光谱差异，以验证模型准确性并指导观测策略。

Method: 使用HD 189733b进行三组模拟：1D大气模型+1D辐射传输、3D GCM模型+1D辐射传输、3D GCM模型+3D辐射传输，结合平衡和非平衡化学过程。

Result: 发现3D模型通常显示较弱的光谱特征，传输光谱SNR为1.66-7.68，发射光谱SNR为5.90-36.93，JWST能够区分主要分子特征的1D和3D模型光谱。

Conclusion: JWST能够区分1D和3D模型光谱，强调了3D建模在解释系外行星大气中的重要性。
...
### [7] [StarEstate: A Python Package for Galactic Population Synthesis](https://arxiv.org/abs/2511.20735)
*Amedeo Romagnolo*
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够快速生成星系合成种群模型的工具，结合统计生成和恒星物理，为解释星系巡天数据提供灵活资源。

Method: 使用优化的预计算逆累积分布函数采样器生成合成种群，结合MESA或快速SSE/BSE演化轨迹，基于年龄依赖的动力学温度概率性地将恒星分配到旋臂。

Result: 能够快速生成合成星系种群，重现年轻示踪物紧密限制在旋臂而老年种群分散的观测特征，并可通过自动分层分类检查特定演化阶段。

Conclusion: StarEstate提供了一个预测多样化恒星天体空间分布的灵活工具，为星系巡天数据的解释提供了有价值的资源。
...
### [8] [A Direct Calibration Algorithm for ADC Interleaving](https://arxiv.org/abs/2511.20763)
...
Conclusion: 这项工作为维持交织ADC系统中的信号保真度提供了稳健且可扩展的解决方案，在需要高速数据采集的领域具有广泛应用前景。
...
### [9] [TriPoDPy: 1D Tri-Population size distributions forDust evolution in protoplanetary disks](https://arxiv.org/abs/2511.20764)
*Nicolas Leo Kaufmann,Thomas Pfeil,Sebastian Stammler,Anna Penzlin,Sandro Christian Paetzold,Til Birnstiel*

Main category: astro-ph.IM

TL;DR: TriPoDPy是一个模拟原行星盘中尘埃演化的代码，包含尘埃增长和动力学，使用参数化尘埃模型，在一维径向网格上演化尘埃分布。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够同时模拟尘埃和气体盘演化的工具，用于研究原行星盘中尘埃的物理过程。

Method: 使用Python编写，核心程序用Fortran90实现，采用一维径向网格和参数化尘埃模型，包含尘埃增长、动力学和气体盘演化。

Result: 开发出TriPoDPy代码，能够模拟尘埃分布演化，并添加了用于成分追踪的示踪剂描述。

Conclusion: TriPoDPy是一个有效的原行星盘尘埃演化模拟工具，为研究尘埃物理过程提供了重要手段。
...
### [10] [The WEAVE-TwiLight-Survey: Expanding WEAVE's Reach to Bright and Low-Surface-Density Targets with a Novel Observing Mode](https://arxiv.org/abs/2511.21184)
...
TL;DR: 开发了WEAVE望远镜的新观测模式，通过叠加多个低目标密度场来提高观测效率，特别适用于亮星观测，并成功应用于WEAVE-TwiLight-Survey项目。
...
Method: 采用启发式方法定义相对于中心导星的天区，通过叠加多个低目标密度场并分配光纤，使用自定义算法选择合适的光纤配置。

Result: 测试观测验证了新观测模式的可行性，能够最小化校准时间，为WEAVE开辟了黄昏时段的观测机会。

Conclusion: 新观测模式扩展了WEAVE的能力，使其能够在不牺牲效率的情况下观测低密度天区配置，为未来行星形成化学模式研究奠定了基础。
...
### [11] [Enhancing Galaxy Classification with U-Net Variational Autoencoders. II. JWST High Redshift Galaxy Sample](https://arxiv.org/abs/2511.21207)
*Sergey Mirzoyan*

Main category: astro-ph.IM

TL;DR: 使用U-Net VAE框架对JWST星系图像进行去噪，提升形态分类准确性，发现在高红移(z>3)星系中盘状结构可能很普遍。


<details>
  <summary>Details</summary>
Motivation: 早期宇宙中的星系由于暗淡和结构复杂，传统分类方法面临挑战，需要去噪技术来识别形态特征。

Method: 采用U-Net变分自编码器框架对JWST星系图像进行去噪处理，使用标准图像质量指标评估去噪效果。

Result: 在292个z=7.69以内的星系样本中，GCNN模型高置信度分类出83个盘状星系，其中70-80%红移大于3，去噪后图像显著提升了多种深度学习模型的分类准确率。

Conclusion: VAE去噪技术可作为分析高红移星系群体的有效预处理步骤，表明早期宇宙中盘状结构可能很常见。
...
### [12] [ALMA Lensing Cluster Survey: Molecular Gas Properties of Line-Emitting Galaxies from a Blind Survey](https://arxiv.org/abs/2511.20748)
...
TL;DR: 使用ALMA透镜星系团巡天数据进行盲搜索，发现了7个发射线星系，包括一个高红移[C II]发射体和多个CO发射体，通过引力透镜效应探测到比以往更低的分子气体质量。


<details>
  <summary>Details</summary>
Motivation: 利用引力透镜效应增强ALMA的探测能力，寻找更暗弱的发射线星系，研究星系中的分子气体性质及其与恒星形成活动的关系。

Method: 采用盲搜索方法分析ALMA透镜星系团巡天数据，识别发射线星系，通过光谱分析确定发射线类型和红移，利用引力透镜放大效应探测低质量分子气体。

Result: 发现7个发射线星系，包括z=6.071的[C II]发射体和多个CO发射体，分子气体质量比以往探测下限低约1个数量级，首次在宇宙学距离上盲探测到CH分子。

Conclusion: 大多数发射体遵循已知的恒星形成主序关系，CH探测表明其可能示踪与活动星系核相关的分子气体，为研究高红移星系分子气体性质提供了新途径。
...
### [13] [The galactic chemical evolution of carbon: Implications for stellar nucleosynthesis](https://arxiv.org/abs/2511.20752)
*Daniel A. Boyea,James W. Johnson,David H. Weinberg*

Main category: astro-ph.GA

TL;DR: 本文通过银河化学演化模型研究碳元素的起源，发现核心坍缩超新星(CCSN)和渐近巨星支(AGB)恒星共同贡献碳元素，其中CCSN产量随金属丰度增加，而AGB产量随金属丰度减少，观测数据支持AGB恒星贡献太阳金属丰度下10-40%的碳元素。


<details>
  <summary>Details</summary>
Motivation: 研究碳元素的宇宙起源，确定核心坍缩超新星和渐近巨星支恒星对碳元素产生的相对贡献，解决当前对这两种来源贡献比例的不确定性。

Method: 使用适用于银河系盘的银河化学演化模型，以APOGEE亚巨星丰度数据作为基准，分析[C/Mg]-[Mg/H]和[C/Mg]-[Mg/Fe]趋势。

Result: 观测显示[C/Mg]随[Mg/H]轻微上升，支持CCSN产量随金属丰度增加而AGB产量随金属丰度减少的情景；[C/Mg]-[Mg/Fe]趋势表明AGB恒星贡献太阳金属丰度下10-40%的碳元素。

Conclusion: 碳元素由CCSN和AGB恒星共同产生，其中CCSN产量随金属丰度增加，AGB产量随金属丰度减少，AGB贡献约10-40%的碳元素，结果与包含旋转的大质量恒星核合成模型一致。
...
### [14] [Ashes of Creation: JWST Uncovers Silicate Dust in Massive Star Clusters](https://arxiv.org/abs/2511.20775)
...
TL;DR: 该论文研究了22个质量超过10^5太阳质量的河外星系星团，发现它们在3-100百万年年龄范围内显示出紧凑的尘埃形态，其中19个星团已清除其诞生尘埃，主要特征是在10μm处有显著增强，可能源于硅酸盐发射。


<details>
  <summary>Details</summary>
Motivation: 理解不同演化阶段星团中尘埃产生的整体机制仍具挑战性，虽然单个天体（如超新星和演化恒星）的尘埃注入机制已被了解，但星团整体的尘埃产生仍难以量化。

Method: 使用JWST-MIRI观测22个大质量河外星系星团，分析其尘埃形态和光谱特征，特别是10μm处的增强现象。

Result: 发现19个星团已清除诞生尘埃，主要特征是在10μm处有显著增强，可能源于硅酸盐发射；最亮的10μm发射源是三个质量最大的星团（>10^6太阳质量），其强度比任何已知恒星源至少强一个数量级。

Conclusion: 10μm处的显著增强可能源于演化恒星（如红超巨星）、特殊恒星类型（如黄特超巨星和明亮蓝变星）的尘埃产生，或超新星在稠密气体中的随机尘埃注入，但最亮的发射源表明存在仅在极端质量和短寿命条件下出现的罕见机制。
...
### [15] [Age and metallicity of low-mass galaxies: from their centres to their stellar halos](https://arxiv.org/abs/2511.20806)
...
TL;DR: 分析低质量星系及其恒星晕的金属丰度和年龄，发现金属丰度径向梯度为负值，且与星系固有属性无关。恒星晕质量-金属丰度关系的弥散可由主要卫星的吸积时间解释。约65%星系呈现U形径向年龄分布，主要由原位恒星物质驱动。更高质量的恒星晕年龄更大，反映了低质量星系演化的复杂性。


<details>
  <summary>Details</summary>
Motivation: 通过分析低质量星系及其恒星晕的金属丰度和年龄，更好地理解这些星系的形成历史。

Method: 使用来自Auriga项目的17个模拟低质量星系（恒星质量范围约3×10^8 M⊙到2×10^10 M⊙），研究这些星系及其恒星晕的金属丰度和年龄，以及这两个属性之间的关系。

Result: 所有星系都有负的径向[Fe/H]梯度；较不活跃矮星系的中心通常比更活跃矮星系的中心更贫金属；金属丰度梯度与星系固有属性（如恒星质量或吸积恒星质量）无相关性；恒星晕质量-金属丰度关系的弥散可由主要卫星的吸积时间解释；约65%星系呈现U形径向平均年龄分布，主要由原位恒星物质驱动；更高质量的恒星晕年龄更大。

Conclusion: 低质量星系及其恒星晕的年龄和金属丰度存在广泛多样性，反映了这些系统可能遵循的复杂且非均匀的演化路径。
...
### [16] [Oscillations of dark matter halos in galaxies](https://arxiv.org/abs/2511.20952)
...
### [17] [Different Origins of Nucleated and Non-nucleated Dwarf Elliptical Galaxies: Identified by the Deep-learning](https://arxiv.org/abs/2511.21010)
*Sanjaya Paudel,Cristiano G. Sabiu,Suk-Jin Yoon,Daya Nidhi Chhatkuli,Woong-Bae G. Zee,Jaewon Yoo,Binod Adhikari*

Main category: astro-ph.GA

TL;DR: 使用机器学习方法在室女座星系团构建了最大的矮椭圆星系样本，发现核化矮椭圆星系更集中在星系团中心区域，而非核化矮椭圆星系分布更广，表明它们分别通过原位形成和外源吸积两种不同机制演化。


<details>
  <summary>Details</summary>
Motivation: 矮椭圆星系是星系团中最主要的星系类型，其中许多包含中央核，这些核是宇宙中最致密的恒星系统。核化和非核化矮椭圆星系的大尺度分布及其起源机制尚未解决。

Method: 使用先进的机器学习框架系统扫描室女座星系团区域（15°×20°），构建了包含2123个矮椭圆星系的同质样本，并进行可靠的核分类。

Result: 核化矮椭圆星系在空间上更加聚集，与质量更大的星系关联更强；而非核化矮椭圆星系分布更广泛，与星系团整体势阱分布更一致。

Conclusion: 大多数核化矮椭圆星系可能在星系团内与质量更大的星系一起形成（原位形成），而非核化矮椭圆星系更可能起源于星系团外部，随后被吸积并重新分布（外源形成）。
...
### [18] [The FAST-SETI Milky Way Globular Cluster Survey I: A Pilot Multibeam On-the-Fly Search of Five Globular Clusters at L-Band](https://arxiv.org/abs/2511.21085)
...
<details>
  <summary>Details</summary>
Motivation: 球状星团是寻找地外技术信号的理想目标，因为它们包含大量古老恒星，可能孕育长期存在的先进文明。这是FAST首次专门针对球状星团的技术信号调查。

Method: 采用多波束点源扫描策略，通过广义目标窗口门控检测，使用turboSETI搜索动态频谱，频率分辨率约7.5Hz，时间分辨率约10秒，漂移率|ν̇|≤4Hz/s，信噪比S/N≥10。

Result: 从约27.5万个原始检测中，经过门控、阵列同时性否决、条纹内排序和单漂移相干性检查，未发现任何技术信号，得到稳健的零结果。

Conclusion: 排除了在观测期间存在高于设定阈值的明亮、持久、各向同性L波段信标的可能性，为在密集恒星系统中开展多历元MBPS活动建立了可扩展的蓝图。
...
### [19] [Scalable and Robust Multiband Modeling of AGN Light Curves in Rubin-LSST](https://arxiv.org/abs/2511.21479)
...
TL;DR: EzTaoX是一个可扩展的光变曲线建模工具，专为利用LSST巡天的多波段观测数据而设计，能够同时表征AGN的UV/光学随机变异性并测量波段间时间延迟。


<details>
  <summary>Details</summary>
Motivation: LSST巡天将监测数千万个活动星系核(AGNs)，产生前所未有的多波段光变曲线数据，但现有工具无法充分利用这些数据的规模、采样频率和多波段特性。

Method: 开发了EzTaoX工具，在CPU上比现有类似工具快约10^2-10^4倍，同时保持相同或更好的模拟变异性恢复精度。

Result: EzTaoX能够对所有LSST发现的AGNs进行连续谱时间延迟测量，并能够在小时时间尺度上稳健表征AGN随机变异性。

Conclusion: EzTaoX为探测AGN吸积流几何结构提供了新机会，并有助于通过独特的变异性特征识别矮星系中的低质量AGNs。
...
### [20] [Molecular Gas and Star Formation in Dwarf Galaxies Observed by the Atacama Large Millimeter/submillimeter Array](https://arxiv.org/abs/2511.21187)
*Kijeong Yim,Soo-Chang Rey*

Main category: astro-ph.GA

TL;DR: 本研究使用ALMA高分辨率CO数据分析了6个近邻矮星系的分子恒星形成定律和引力不稳定性，发现矮星系与大型螺旋星系具有相似的分子恒星形成定律斜率，表明恒星形成的基本物理过程在不同质量星系中具有普适性。


<details>
  <summary>Details</summary>
Motivation: 研究矮星系中的恒星形成过程，探索低质量、低金属丰度星系是否遵循与大型星系相同的恒星形成物理规律，填补对矮星系分子气体与恒星形成关系研究的空白。

Method: 使用ALMA高分辨率CO(J=1→0)数据，结合GALEX近紫外和WISE 12μm成像数据估算恒星形成率，在数百秒差距尺度上分析分子气体密度与恒星形成率密度的关系，并计算Toomre Q参数。

Result: 分子恒星形成定律的幂律斜率范围为0.62-1.08，平均值为0.81±0.18，排除CO数据约束较差的星系后为0.87±0.05；Toomre Q参数在盘面各处接近1，径向变化很小。

Conclusion: 尽管矮星系质量较小、金属丰度较低，但其恒星形成受与大型星系相同的基本物理过程支配，高分辨率分子气体观测对理解低质量星系至关重要。
...
### [21] [Molybdenum and ruthenium in the Galactic disk: A closer look at their nucleosynthesis components](https://arxiv.org/abs/2511.21190)
*Tamara Mishenina,Teresa Kurtukian-Nieto,Tatiana Gorbaneva,Anish M. Amarsi,Athanasios Psaltis,Marco Pignatari*

Main category: astro-ph.GA

TL;DR: 该研究通过观测154颗银河系盘面巨星中的钼和钌元素丰度，结合锶和锆元素分析，探讨了这些中子俘获元素的核合成起源。研究发现[Mo/Fe]和[Ru/Fe]随[Fe/H]增加而下降，与[Zr/Fe]模式相似，而[Sr/Fe]则保持相对平坦。观测到的[Zr/Mo]和[Ru/Mo]散射可能来自r-过程和s-过程的共同贡献。


<details>
  <summary>Details</summary>
Motivation: 钼和钌元素的恒星起源仍存在争议，研究它们的丰度能为核合成过程和更广泛的中子俘获元素演化提供重要见解。

Method: 对银河系盘面154颗巨星进行观测，在局部热力学平衡假设下通过拟合合成光谱确定元素丰度。使用Mo I 5506和5533 Å谱线确定钼丰度，使用Ru I 4584和4757 Å谱线确定钌丰度。

Result: [Mo/Fe]和[Ru/Fe]随[Fe/H]增加呈下降趋势，与[Zr/Fe]模式相似，而[Sr/Fe]保持相对平坦。所有恒星都显示出较低的[Zr/Mo]和较高的[Ru/Mo]，符合经典核合成预期。观测到的[Zr/Mo]和[Ru/Mo]散射与低金属度r-II恒星中观察到的变化一致。

Conclusion: 观测到的[Zr/Mo]和[Ru/Mo]散射可能来自r-过程和s-过程的共同贡献，但仍不确定是否需要中微子风组分或i-过程的额外贡献来解释银河系盘面中的观测散射。未来需要约束r-II恒星中的[Zr/Mo]和[Ru/Mo]散射来确定是否需要额外的核合成组分贡献。
...
### [22] [Galaxy evolution in the post-merger regime. IV - The long-term effect of mergers on galactic stellar mass growth and distribution](https://arxiv.org/abs/2511.21512)
*Sara L. Ellison,Leonardo Ferreira*

Main category: astro-ph.GA

TL;DR: 该研究通过分析约14,000个已完成合并的星系样本，直接测量了星系合并引发的恒星质量增长，发现中央区域存在10-20%的恒星质量增强，且这种增长延伸到约7千秒差距的较大范围。


<details>
  <summary>Details</summary>
Motivation: 星系合并被认为会触发中央恒星形成爆发，但对此'爆发质量分数'的观测测量很少。研究旨在直接测量合并诱导的恒星质量增长，而不依赖恒星种群模型或光轮廓拟合。

Method: 组装了约14,000个已完成合并的星系样本，与匹配的对照组进行比较。使用固定角孔径和星系核球测量中央恒星质量，并对61个z<0.05的星系使用MaNGA积分视场单元数据进行千秒尺度评估。

Result: 发现合并后星系中央区域存在10-20%的恒星质量过剩，且这种质量增长延伸到约7千秒差距(1.4倍有效半径)的范围，而非先前认为的仅局限于非常紧凑的中央区域。

Conclusion: 这是首次独立于恒星种群模型或光轮廓拟合的直接测量，证明在晚期合并后星系中存在显著且扩展的质量积累。
...
### [23] [An empirical view of the extended atmosphere and inner envelope of the asymptotic giant branch star R Doradus. II. Constraining the dust properties with radiative transfer modelling](https://arxiv.org/abs/2511.20816)
...
TL;DR: 该论文研究了富氧AGB星R Doradus的星风驱动机制，发现仅靠尘埃光子散射不足以驱动星风，需要其他机制参与。


<details>
  <summary>Details</summary>
Motivation: 目前对富氧AGB星质量损失的理解不足，现有模型认为大颗粒尘埃通过光子散射驱动星风，但这种颗粒是否存在足够数量仍不确定。

Method: 结合偏振测量约束、元素丰度限制和力平衡计算，使用SPHERE/ZIMPOL的高角分辨率偏振观测数据，通过RADMC-3D模拟星周尘埃，探索颗粒大小、密度结构和波长相关恒星半径的广泛参数空间。

Result: 虽然多种尘埃配置能重现观测到的散射模式，但在实际气尘比下，即使考虑最大元素消耗，也没有任何配置能产生足够的辐射力。

Conclusion: 对于R Doradus星，仅靠尘埃光子散射无法驱动星风，暗示必须有其他机制参与星风形成过程。
...
### [24] [Measuring Dust Masses of Protoplanetary Disks in Serpens and L1641/L1647 with ALMA](https://arxiv.org/abs/2511.20817)
*Luisa F. Zamudio-Ruvalcaba,Catherine C. Espaillat,Álvaro Ribas,Enrique Macías*

Main category: astro-ph.SR

TL;DR: 本文通过SED建模和多波长数据分析178颗T-Tauri星的盘尘埃质量，发现计算出的盘质量比先前报道的高约2倍，原因是盘在毫米波长可能部分光学厚，而传统方法假设盘在1.3毫米光学薄。


<details>
  <summary>Details</summary>
Motivation: 原行星盘是行星形成过程中的关键组成部分，其尘埃和气体含量限制了系统中可形成行星的数量和大小。准确测量盘尘埃质量对理解行星形成过程至关重要。

Method: 使用光谱能量分布(SED)建模和多波长数据（包括文献中的1.3毫米ALMA波段6流量）分析178颗T-Tauri星（18颗在Serpens，160颗在L1641/L1647）的盘尘埃质量。

Result: 计算出的盘尘埃质量比先前报道的高约2倍，这是因为盘在毫米波长可能部分光学厚，而传统方法假设盘在1.3毫米光学薄。对Serpens和L1641/L1647盘样本子集计算1.3和7毫米光学深度，显示绝大多数盘在较长毫米波长变得光学薄。

Conclusion: 盘在1.3毫米可能部分光学厚，导致传统方法低估盘尘埃质量。7毫米（ALMA波段1）观测对于更准确表征盘尘埃质量至关重要。
...
### [25] [Automated all-sky detection of γ Doradus / δ Scuti hybrids in TESS data from positive unlabelled (PU) learning](https://arxiv.org/abs/2511.20908)
*Mykyta Kliapets,Pablo Huijse,Andrew Tkachenko,Alex Kemp,Dario J. Fritzewski,Daniel Hey,Conny Aerts*

Main category: astro-ph.SR

TL;DR: 该研究使用TESS卫星数据，通过特征驱动的正未标记学习方法，在全天AF型恒星中发现了62,026个新的γ Dor/δ Sct混合脉动星候选体，并验证了TESS-Gaia光变曲线在变星研究中的有效性。


<details>
  <summary>Details</summary>
Motivation: 利用TESS卫星观测数据寻找先前未被发现的γ Dor/δ Sct混合脉动星，这些恒星是研究恒星内外层的理想实验室，同时评估TESS-Gaia光变曲线在变星研究中的适用性。

Method: 比较Kepler、QLP和TGLC数据中已确认混合脉动星的主频和次频，使用特征驱动的正未标记学习分类器在TESS AF型恒星中搜索新混合脉动星，并分析检测到的种群特性。

Result: TGLC数据的变异性与QLP光变曲线高度一致，对Kepler提取频率的恢复率很高。智能分箱方法从大型未标记数据集中稳健提取混合脉动星，测试集混合脉动星的平均袋外预测准确率达93.04%。在高概率候选体中发现了更多压力模式主导的混合脉动星。

Conclusion: 成功构建了包含62,026个新候选光变曲线的目录，为每个可用扇区提供了个体成为混合脉动星的概率，证明了TGLC数据在变星研究中的可靠性和实用性。
...
### [26] [High-order Gravity-mode Period Spacing Patterns of Intermediate-mass ($1.5 \, M_\odot < M < 3 \, M_{\odot}$) Main-sequence Stars I. Perturbative Analysis](https://arxiv.org/abs/2511.20948)
*Yoshiki Hatta,Takashi Sekii*

Main category: astro-ph.SR

TL;DR: 本文通过一阶微扰分析研究了Brunt-Väisälä频率的有限宽度过渡对高阶重力模式周期间距模式的影响，并推导出解析表达式。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解中等质量主序星重力模式脉动星的内部特性，需要研究高阶重力模式周期间距模式的理论特性。

Method: 采用一阶微扰分析方法，将Brunt-Väisälä频率二阶导数中的凸起视为小扰动，推导出周期间距模式的解析表达式。

Result: 解析表达式显示振荡周期间距模式的振幅由BV频率二阶导数凸起的加权平均值决定，测试表明该表达式能较好地重现数值计算的周期间距模式。

Conclusion: 微扰分析结果有助于改进周期间距模式的半解析表达式，从而能够通过SPB星和γ Dor星的周期间距模式推断化学组成剖面和旋转速率。
...
### [27] [Refined classification of YSOs and AGB stars by IR magnitudes, colors, and time-domain analysis with machine learning](https://arxiv.org/abs/2511.21012)
*Hyunwook Jheonn,Jeong-Eun Lee,Jinho Lee,Seonjae Lee,Hyeyoon Lee,ShinGeon Kim,Carlos Contreras Peña,Mi-Ryang Kim*

Main category: astro-ph.SR

TL;DR: 提出双过滤器模型，利用机器学习和深度学习方法，结合光变曲线数据对年轻恒星天体（YSOs）和渐近巨星支（AGB）恒星进行二元分类，以解决它们在红外测光中的误分类问题。


<details>
  <summary>Details</summary>
Motivation: YSOs和AGB恒星由于相似的温度和环星尘埃存在，在红外测光中具有相似特征，难以区分且经常被误分类。虽然已有机器学习方法尝试减少误分类，但实现可靠分离仍然具有挑战性。

Method: 开发双过滤器模型，利用YSOs和AGB恒星由不同变异性机制产生的不同光变曲线特征，结合机器学习和深度学习技术进行分类。

Result: 该方法发现了红外测光中被误分类的YSOs和AGB恒星，并在金牛座YSOs和光谱确认的AGB恒星上进行了验证。应用于SPICY星表进行星表精化，识别出潜在的AGB恒星污染物。

Conclusion: 双过滤器模型通过利用光变曲线数据有效提高了YSOs和AGB恒星的分类准确性，为天体分类提供了新的可靠方法。
...
### [28] [Multiwavelength Observations for a Double-decker Filament Channel in AR 13102](https://arxiv.org/abs/2511.21062)
*Yin Zhang,Baolin Tan,Quan Wang,Jing Huang,Zhe Xu,Kanfan Ji,Xiao Yang,Jie Chen,Xianyong Bai,Zhenyong Hou,Yuanyong Deng*

Main category: astro-ph.SR

TL;DR: 通过极紫外和Hα波段观测发现双层层叠暗条通道的存在证据，展示了暗条形成、磁场重联触发爆发以及热等离子体流动揭示上层暗条通道骨架的过程。


<details>
  <summary>Details</summary>
Motivation: 研究双层层叠暗条通道的观测证据，了解其磁场结构和等离子体填充过程，这对于理解太阳大气中复杂磁场结构的形成和演化具有重要意义。

Method: 使用极紫外和Hα波段的观测数据，分析暗条通道的结构特征、等离子体流动和磁场重联过程，通过追踪亮化特征和暗条形态变化来推断磁场结构。

Result: 观测证实了双层层叠暗条通道的存在，下层通道表现为间歇性暗条，上层通道仅东部填充暗等离子体。重联过程导致暗条连接形成新暗条，并触发下层暗条的部分爆发，同时热等离子体流动揭示了上层通道的完整骨架。

Conclusion: 双层层叠暗条通道作为一种磁场结构，在完全填充热/冷等离子体之前就已存在，观测结果支持了这种复杂磁场结构在太阳大气中的存在和演化模型。
...
### [29] [Shock-type inference of L1157 B2 using methanol desorption](https://arxiv.org/abs/2511.21234)
*Cedric Baijot,Maria Groyne,Michaël De Becker*

Main category: astro-ph.SR

TL;DR: 该研究通过模拟不同速度的冲击波模型，分析分子丰度来诊断L1157 B2原恒星对象的冲击类型，发现甲醇解吸百分比是区分非辐照C型和J型冲击的有效指标。


<details>
  <summary>Details</summary>
Motivation: 低速度分子外流的冲击类型通常不明确，现有研究在比较低速度和高速度外流时未充分考虑冲击类型差异，需要开发可靠的冲击类型诊断方法。

Method: 使用Paris-Durham冲击代码模拟5-19 km/s速度范围和10^2-10^5 cm^-3低密度下的冲击模型网格，计算甲醇解吸百分比，并与L1157 B2观测数据及四个已知冲击基准进行比较。

Result: L1157 B2被诊断为非辐照C型冲击，甲醇解吸百分比能有效区分冲击类型，该方法在基准测试中表现出良好适用性。

Conclusion: 甲醇解吸百分比可作为诊断非辐照区域冲击类型的通用方法，L1157 B2被确定为非辐照C型冲击，其速度和密度参数与先前研究一致。
...
### [30] [Resonance locking and tidal evolution in rotating γ-Doradus binaries](https://arxiv.org/abs/2511.21347)
*L. Fellay,M. -A. Dupret,P. A. Kołaczek-Szymański*

Main category: astro-ph.SR

TL;DR: 研究双星系统中逆潮汐和共振锁定现象，开发了轨道演化与恒星振荡耦合的代码，发现共振锁定能长期稳定存在并显著影响角动量交换。


<details>
  <summary>Details</summary>
Motivation: 理解双星系统演化中的潮汐相互作用，特别是传统辐射阻尼机制之外的逆潮汐现象和共振锁定对轨道演化的潜在重要影响。

Method: 开发了轨道演化代码与恒星振荡代码耦合的系统，实时计算包含多个同时强迫频率的动态潮汐对旋转和轨道演化的影响。

Result: 共振锁定可长期稳定存在，对快速旋转恒星可增加约70%角动量；对慢速旋转恒星可减缓系统旋转演化，有效驱动轨道与旋转频率不同步。

Conclusion: 共振锁定是双星系统中长期角动量交换的重要机制，能显著影响恒星旋转演化和系统同步状态。
...
### [31] [Variability of H$α$ chromospheric activity of solar-like stars revealed by the time-domain data of LAMOST Medium-Resolution Spectroscopic Survey](https://arxiv.org/abs/2511.21511)
*Han He,Ali Luo,Haotong Zhang,Song Wang*

Main category: astro-ph.SR

TL;DR: 本研究利用LAMOST中分辨率光谱巡天数据，分析了类太阳恒星的Hα色球活动变异性。研究发现Hα活动强度较低的恒星与较高恒星在变异性行为上存在明显差异，前者Hα活动呈现长期规律变化，后者则多为随机波动。


<details>
  <summary>Details</summary>
Motivation: 研究类太阳恒星Hα色球活动的变异性特征，探索不同活动强度恒星在Hα活动变化模式上的差异。

Method: 使用LAMOST中分辨率光谱巡天数据，定义R_Hα指数（Hα光度与热光度之比）衡量活动强度，用多次观测的R_Hα中值代表恒星活动强度，用R_Hα波动范围表示变异性。

Result: 发现类太阳恒星的Hα变异性比活动强度小约一个数量级。低活动强度恒星（log R_Hα^median < -4.85）的分布上包络线先增后减，而高活动强度恒星则呈正相关。低活动强度恒星的大变异性天体呈现长期规律变化，高活动强度恒星则多为随机波动。

Conclusion: 类太阳恒星的Hα色球活动变异性与活动强度密切相关，不同活动强度的恒星展现出不同的变化模式，这为理解恒星活动机制提供了重要线索。
...
### [32] [Formation and nature of Huntsman binary pulsars](https://arxiv.org/abs/2511.21589)
*O. G. Benvenuto,M. A. De Vito,M. Echeveste,M. L. Novarino,N. D. Pires,L. M. de Sá,J. E. Horvath*

Main category: astro-ph.SR

TL;DR: 本文研究了蜘蛛系统（包含中子星和伴星的双星系统）的演化，特别关注了Huntsman群组的形成机制。通过考虑辐照反馈和氢壳层燃烧分离效应，证明了Huntsman是蜘蛛系统演化中的一个预期阶段。


<details>
  <summary>Details</summary>
Motivation: 新的观测扩展了蜘蛛系统的分类，增加了Huntsman群组。本文旨在通过明确的演化轨迹来支持最近提出的Huntsman性质，并讨论整个蜘蛛系统类别如何从辐照和剥离风的完整考虑中产生。

Method: 使用双星演化代码计算了由伴星和中子星组成的双星系统，针对不同的初始轨道周期，假设太阳成分和Z=0.01。同时处理辐照反馈效应和氢壳层燃烧分离效应。

Result: 发现Huntsman脉冲星与演化阶段（由于氢壳层燃烧分离动力学）系统保持分离数百万年的关联是合理的。包含辐照反馈的模型显示出可以自然地与Redback阶段相关联的分离事件。辐照反馈不会阻止或修改氢壳层燃烧分离。

Conclusion: Huntsman是蜘蛛系统在相当一般条件下的预期演化阶段，这是朝着蜘蛛脉冲星统一图像迈出的另一步。
